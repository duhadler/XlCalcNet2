...
class LbfgsSolver : public ISolver<ProblemType, 1> {
  public:
    ...
    void minimize(ProblemType &objFunc, TVector &x0) {
        ...
        Scalar eps8 = sqrt(std::numeric_limits<Scalar>::epsilon());
        Scalar eps4 = eps8;
        //Scalar eps4 = sqrt(eps8);
        do {
            //const Scalar relativeEpsilon = static_cast<Scalar>(0.0001) * std::max<Scalar>(static_cast<Scalar>(1.0), x0.norm());
            const Scalar relativeEpsilon = static_cast<Scalar>(eps4) * std::max<Scalar>(static_cast<Scalar>(1.0), x0.norm());
            ...
            q = grad;
            const int k = std::min<size_t>(m, iter);
            ...
            if (descent > -eps4 * relativeEpsilon) {
                    q = -1 * grad;
                iter = 0;
                alpha_init = 1.0;
            }
            ...
            H0k = y.dot(s) / static_cast<Scalar>(y.dot(y));
