...
#if defined(Use_Single) || defined(Use_Double)  || defined(Use_LongDouble)
...
#else
      const Scalar tt1 = std::max<Scalar>(Scalar(1.e-04), 10 * nextafter(f[index[0]], std::numeric_limits<Scalar>::epsilon()) - f[index[0]]);
      const Scalar tt2 = std::max<Scalar>(Scalar(1.e-04), 10 * (nextafter(x0.col(index[0]).maxCoeff(), std::numeric_limits<Scalar>::epsilon())
                    - x0.col(index[0]).maxCoeff()));
#endif
