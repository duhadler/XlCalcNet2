...
namespace MPFR {
	...
	mpfr_ptr  MpfrClass::_ptr() { return this->mpfr_rep; }
	...
	void Prod_H1(MpfrClass& r, long int& k,
		const MpfrClass& x, const MpfrClass& y, RoundingMode rnd)
		// r*2^k = x*y;  MaxFloat()/8 <= |r| < MaxFloat()/2.098....;
		// r = 0, r = NaN, r = +inf,  r = -inf  --->   k = 0;
	{
		const long int p = 1073741822;
		k = 0;
		if (isNumber(x) && isNumber(y))
			if (sign(x)*sign(y) != 0)
				k = expo(x) + expo(y) - p;
		...
		// Rechnung in mehr als doppelter Genauigkeit, um Auslöschung möglichst zu vermeiden
	{
		...
		MpfrClass::SetCurrPrecision(prec + (k_ab - k_cd)); // Diese noch groessere Current-Precision
		// garantiert zusaetzlich die rundungsfehlerfrei Berechnung der nachfolgenden Summe:
	...
	{
		MpfrClass res(0);
		mpfr_rootn_ui(res.mpfr_rep, r.mpfr_rep, k, rnd);
	...
	MpfrClass::operator int() const
	{
		//return static_cast<int>(1);
		return static_cast<int>(mpfr_get_si(&(*this->mpfr_rep), GMP_RNDZ));
